...
export function computeDeliveryEvaluation(pickedPackages, deliveryTime, deliveryPosition, state, config = null) {
  return evaluateDelivery({
    state,
    packages: pickedPackages,
    deliveryTime,
    deliveryPosition,
    config: config ?? state?.params ?? DEFAULT_PARAMS
  });
}

export function computeDeliveredValue(pickedPackages, deliveryTime, deliveryPosition, state, config = null) {
  return computeDeliveryEvaluation(pickedPackages, deliveryTime, deliveryPosition, state, config).value;
}
...
export function extendToRed(plan, red, _state, oracle, _config) {
  ...
  const delivery = computeDeliveryEvaluation(plan.pickedPackages, deliveryTime, red.position, _state, _config);
  if (!delivery.allowed) return null;
...
export function carriedPotential(plan, _state, oracle, config = null) {
  ...
  for (const red of reds) {
    ...
    const deliveryTime = plan.time + edge.cost;
    const delivered = computeDeliveredValue(plan.pickedPackages, deliveryTime, red.position, _state, config);
...
export function bestCompletionValue(plan, reds, state, oracle, config) {
  ...
  for (const red of reds) {
    ...
    if (!edge || !Number.isFinite(edge.cost)) continue;
    const deliveryTime = plan.time + edge.cost;
    const delivered = computeDeliveredValue(plan.pickedPackages, deliveryTime, red.position, state, config);
