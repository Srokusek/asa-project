...
test("visible sensing marks tile and green observation timestamps", () => {
  const beliefs = setupBeliefs();
  beliefs.updateParcelsSensing([], [{ x: 0, y: 0 }, { x: 1, y: 0 }]);

  assert.equal(beliefs.lastObservedAtByTile.get("0,0"), beliefs.time);
  assert.equal(beliefs.lastObservedAtByTile.get("1,0"), beliefs.time);
  assert.equal(beliefs.lastObservedAtByGreen.get("1,0"), beliefs.time);

  beliefs.advanceTime(3);
  assert.equal(beliefs.tileStaleness({ x: 0, y: 0 }), 3);
  assert.equal(beliefs.greenStaleness({ x: 1, y: 0 }), 3);
});
...
test("temporary block marks planner state cell as non-walkable without changing tile belief", () => {
  const config = testConfig();
  const beliefs = new BeliefState(config);
  beliefs.updateMap(3, 1, [
    { x: 0, y: 0, type: "3" },
  ...
  beliefs.updateSelf({ id: "ME", name: "me", x: 0, y: 0, score: 0, penalty: 0 });
  beliefs.markTemporaryBlocked({ x: 1, y: 0 }, 3, "test");

  const plannerState = buildPlannerState(beliefs, config);

  assert.equal(beliefs.tiles.get("1,0").type, "3");
  assert.equal(isWalkable(plannerState, { x: 1, y: 0 }), false);
});

test("temporary blocks expire as belief time advances", () => {
  const beliefs = setupBeliefs();
  const initialBlockVersion = beliefs.temporaryBlockVersion;
  beliefs.markTemporaryBlocked({ x: 1, y: 0 }, 1, "test");
  assert.equal(beliefs.isTemporarilyBlocked({ x: 1, y: 0 }), true);
  assert.equal(beliefs.temporaryBlockVersion, initialBlockVersion + 1);

  beliefs.advanceTime();

  assert.equal(beliefs.isTemporarilyBlocked({ x: 1, y: 0 }), false);
  assert.equal(beliefs.temporaryBlockVersion, initialBlockVersion + 2);
});
...
test("planner state carries observation maps and sensing range", () => {
  const config = testConfig();
  const beliefs = setupBeliefs();
  beliefs.updateParcelsSensing([], [{ x: 1, y: 0 }]);
  const plannerState = buildPlannerState(beliefs, config);

  assert.equal(plannerState.lastObservedAtByTile["1,0"], beliefs.time);
  assert.equal(plannerState.lastObservedAtByGreen["1,0"], beliefs.time);
  assert.equal(plannerState.sensingRange, config.planner.sensingRange);
});
